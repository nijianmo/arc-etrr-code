...
class TreeAttention(Model):
    ...
    def forward(self,
        ...
        nodes_mask = get_text_field_mask(nodes, 1).float() # add num_wrapping_dims=1 and convert into float
    ...
    def _get_edge_probabilities(self, encoded_premise, mean_node_premise_attention, edge_sources,
        ...
        masked_edge_labels = replace_masked_values(edge_labels.unsqueeze(-1).float(), edge_mask, 0).squeeze(
            2).long()
